...
import { addData, pool, addCustomerData, addUserToDatabase } from "./create_tables1.js";
    ...
    pool.query(findUserIdQuery, [username.username], (err, result) => {
        if (err) {
            ...
            if (result.length === 0) {
                ...
                INSERT INTO apo_test_data_model.customer
                (Customer_id, Customer_name, Password, User_id)
                VALUES (?, ?, 'dfd', ?);
              `;
